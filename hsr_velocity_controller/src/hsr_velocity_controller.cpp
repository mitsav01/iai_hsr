...
public:
    ...
    {
        // No need to create a new node; get_node() provides lifecycle node
        return controller_interface::CallbackReturn::SUCCESS;
    }

    controller_interface::CallbackReturn on_configure(const rclcpp_lifecycle::State & /*previous_state*/) override
    {
        // Get parameters from launch/YAML
        joint_names_ = get_node()->declare_parameter<std::vector<std::string>>("joints");
        p_gains_ = get_node()->declare_parameter<std::vector<double>>("p_gains");
        i_gains_ = get_node()->declare_parameter<std::vector<double>>("i_gains");
        d_gains_ = get_node()->declare_parameter<std::vector<double>>("d_gains");
        ff_gains_ = get_node()->declare_parameter<std::vector<double>>("feedforward_gains");

        if (joint_names_.empty())
        {
            RCLCPP_ERROR(get_node()->get_logger(), "No joints specified in parameter file!");
        ...
        // Subscriber
        sub_command_ = get_node()->create_subscription<std_msgs::msg::Float64MultiArray>(
        ...
        pub_ = std::make_unique<realtime_tools::RealtimePublisher<std_msgs::msg::Float64MultiArray>>(
            get_node()->create_publisher<std_msgs::msg::Float64MultiArray>("controller_state", 1));

        RCLCPP_INFO(get_node()->get_logger(), "HsrVelocityController Configured.");
        return controller_interface::CallbackReturn::SUCCESS;
    }
    ...
    controller_interface::CallbackReturn on_activate(const rclcpp_lifecycle::State & /*previous_state*/) override
    {
        ...
        {
            joint_handles_.push_back(hardware_interface::CommandInterface(joint_name, hardware_interface::HW_IF_POSITION));
        }
        return controller_interface::CallbackReturn::SUCCESS;
    }
    ...
    controller_interface::return_type update(const rclcpp::Time & /*time*/, const rclcpp::Duration &period) override
    {
        ...
        for (size_t i = 0; i < n_joints_; i++)
        {
            ...
            {
                RCLCPP_ERROR(get_node()->get_logger(), "Failed to get value for joint %s", joint_names_[i].c_str());
                continue;
            }
            ...
            {
                RCLCPP_ERROR(get_node()->get_logger(), "Failed to set value for joint %s", joint_names_[i].c_str());
            }
        ...
        // Publish controller state at lower frequency
...
private:
    void commandCB(const std_msgs::msg::Float64MultiArray::SharedPtr msg)
    {
        ...
        {
            RCLCPP_ERROR(get_node()->get_logger(), "Command size mismatch: %zu (expected %u)", msg->data.size(), n_joints_);
            return;
        }
